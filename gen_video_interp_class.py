...
def gen_interp_video(G, mp4: str, seeds, shuffle_seed=None, w_frames=60*4, kind='cubic', grid_dims=(1,1), num_keyframes=None, wraps=2, psi=1, device=torch.device('cuda'), **video_kwargs):
    ...
    # random classes
    classes = [np.random.randint(0, 1000) for _ in range(len(seeds))]
    classes.append(classes[0])

    for i in range(w_frames):
        current_interval = i // (w_frames // len(seeds))
        interp = i / (w_frames // len(seeds))
        label[i * len(label) // w_frames, classes[current_interval]] = 1 - interp
        label[i * len(label) // w_frames, classes[current_interval + 1]] = interp
